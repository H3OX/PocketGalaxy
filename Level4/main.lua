...
local qy=0
local v=0
local music = audio.loadStream('music4.mp3')
...
star = display.newImage('star.png', display.contentCenterX,
display.contentCenterY+math.random(-70, 70))
star:scale(0.1, 0.1)
...
mars = display.newImage(grp2, 'mars.png', star.x-255, star.y-255)
...
saturn = display.newImage(grp4, 'saturn.png', star.x-160, star.y-160)
physics.addBody(saturn, 'kinematic', {radius=22, isSensor=true})
saturn:scale(0.04, 0.04)
transition.to(grp4,{ time = 100000, rotation = 17500, iterations = -1})



grp5 = display.newGroup()
grp5.x, grp5.y = star.x+25, star.y+15
venus = display.newImage(grp5, 'venus.png', star.x-230, star.y-230)
physics.addBody(venus, 'kinematic', {radius=22, isSensor=true})
venus:scale(0.04, 0.04)
transition.to(grp5,{ time = 100000, rotation = 12000, iterations = -1})
    ...
    if ( event.phase == "began" and objectToPull.touchJoint == nil ) then
         timer.performWithDelay( 100,
            function()
                ...
                objectToPull.touchJoint.frequency = 0.35
                objectToPull.touchJoint.dampingRatio = 0.1
